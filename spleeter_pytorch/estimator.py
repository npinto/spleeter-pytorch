...
class Estimator(nn.Module):
    def __init__(self, num_instrumments, checkpoint_path):
        ...
        # XXX: hacky
        if '5stems' in checkpoint_path:
            output_mask_logit = True
        else:
            output_mask_logit = False

        if '4stems' in checkpoint_path or '5stems' in checkpoint_path:
            elu = True
        else:
            elu = False

        print(checkpoint_path, output_mask_logit, elu)
        ...
        for i in range(num_instrumments):
            ...
            net = UNet(2, output_mask_logit=output_mask_logit, elu=elu)
            ckpt = ckpts[i]
    ...
    def separate(self, wav):
        ...
        #import IPython; ipshell = IPython.embed; ipshell(banner1='ipshell')
        #masks = list(nn.functional.softmax(torch.cat(masks), dim=0).unsqueeze(1))
        #import IPython; ipshell = IPython.embed; ipshell(banner1='ipshell')
        masks = list((torch.softmax(torch.cat(masks), axis=0) * stft_mag).unsqueeze(1))
        #import IPython; ipshell = IPython.embed; ipshell(banner1='ipshell')
